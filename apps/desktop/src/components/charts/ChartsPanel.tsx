  ...
  const title =
    chart.kind === "histogram"
      ...
      : chart.kind === "scatter"
        ...
        : chart.kind === "box"
          ? `박스플롯 · ${chart.x}`
          : `Moran 산점도 · ${chart.x}${chart.y ? ` × ${chart.y}` : ""}`;
  return (
    <section className="chart-card" data-testid="chart-card">
      <header>
        ...
        <button className="link" onClick={() => removeChart(chart.id)} aria-label="차트 닫기">
          ✕
        </button>
      </header>
...
function ColumnChart({ ds, chart }: { ds: LoadedDataset; chart: ChartSpec }) {
  ...
  if (!data) return <div className="chart-loading muted">불러오는 중…</div>;
...
function MoranBody({ ds, chart }: { ds: LoadedDataset; chart: ChartSpec }) {
  const m = chart.moran;
  if (!m) return <div className="chart-loading muted">계산 중…</div>;
  const lagLabel = `W·${m.column_y ?? m.column} (표준화)`;
  ...
  return (
    <>
      <div className="stat-row" data-testid="moran-stats">
        ...
        <div className="stat">
          <span className="label">유사 p ({m.permutations}회)</span>
          {p(m.p_sim)}
        </div>
        <div className="stat">
      ...
      <Scatter ds={ds} xs={m.z} ys={m.lag} xLabel={`${m.column} (표준화)`} yLabel={lagLabel} moran />
      <div className="muted small pad-x">가중치 {m.weights} · 순열 기준 분포 (노란 선이 관측값 I)</div>
