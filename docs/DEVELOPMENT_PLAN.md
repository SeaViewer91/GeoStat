...
### P6. 배포 — 0.9 공개 베타 (2026-09-24)

- [x] 릴리스 자동화 (`.github/workflows/release.yml`): v 태그 푸시 → macOS 러너에서 엔진 빌드·스모크 테스트 → 엔진 ad-hoc 서명 → Tauri 앱·dmg → GitHub Release 업로드. 수동 실행은 아티팩트만 남김
  - `scripts/version.py`로 앱·엔진 버전 5곳을 한꺼번에 바꾸고, 워크플로가 태그와 버전이 같은지 확인함
  - 릴리스 노트는 `CHANGELOG.md`에서 뽑고 설치 안내(`packaging/release-install-note.md`)를 붙임. `SHA256SUMS` 함께 올림
- [x] 자동 업데이트 (tauri-plugin-updater): 시작 4초 뒤 조용히 확인, 새 버전이면 알림 → 받아서 서명 검증 → 설치 → 다시 시작. 도움말 → 업데이트 확인
  - 업데이트 서명키는 Tauri 자체 키(minisign)임. 공개키는 `tauri.conf.json`, 개인키·암호는 GitHub 비밀값에 있음 (Apple 인증서와 무관)
  - 매니페스트: `releases/latest/download/latest.json` → 사전 배포판(prerelease)은 업데이트 대상이 아님
- [x] 엔진 비정상 종료 알림: Rust가 `engine-exited` 이벤트를 보내고 앱이 "엔진 다시 시작" 버튼을 띄움 (`restart_engine` 명령). 다시 띄우면 화면 상태를 비움
- [x] 한/영 화면 (gettext 방식, `src/i18n`): 한국어 문구가 키이고 영어 사전 604개. 엔진이 보내는 고정 라벨(범례·보고서 요약·진단 이름)도 번역함. `npm run i18n:check`로 CI에서 빠진 문구를 막음
  - 엔진 오류 메시지·보고서 참고 문장·텍스트 보고서는 한국어로만 나옴 (알려진 제약)
- [x] 사용 설명서 (`docs/manual.md`, 영어 요약 `docs/manual.en.md`), 도움말 메뉴에서 엶
- [x] 샘플 데이터 4종을 엔진에 포함 (조지아 GWR, 노스캐롤라이나 SIDS, 서울 가상 격자, 서울 가상 지형). 도움말에서 고르면 문서 폴더로 복사해서 엶 (앱 번들 안 원본을 건드려 서명이 깨지지 않게 함)
  - 조지아 GWR 대역폭 117·R² 0.678이 mgwr 문서와 일치함을 테스트로 확인
- [x] P1 이월분: 속성 조건 선택, 지도 이미지(PNG) 저장(범례·출처 포함)
